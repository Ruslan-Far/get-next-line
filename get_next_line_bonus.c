...
/*   Updated: 2021/11/12 15:51:14 by ljaehaer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	static char	*line[FD_SIZE] = {0};

	if (BUFFER_SIZE <= 0)
		return (NULL);
	if (!line[fd])
	{
		line[fd] = malloc(sizeof(char));
		*line[fd] = '\0';
	}
	if (ft_strchr(line[fd], '\n'))
		return (ft_process_line_at_empty_buf(&line[fd]));
	while (1)
	{
		ft_bzero(buffer, BUFFER_SIZE + 1);
		read_bytes = read(fd, buffer, BUFFER_SIZE);
		if (read_bytes <= 0)
			return (multi_func(&line[fd], read_bytes));
		if (ft_strchr(buffer, '\n'))
			return (ft_process_line(buffer, &line[fd]));
